...
export default function LoginPage() {
  ...
  async function onSubmit(values: z.infer<typeof schema>) {
    ...
    if (!result?.error) router.push("/account");
  }
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto max-w-md space-y-3 rounded border p-4">
      ...
      <button type="button" className="w-full rounded border border-[#006847] py-2 text-[#006847]" onClick={() => signIn("google")}>
        Continue with Google
      </button>
      <button type="button" className="w-full rounded border border-[#006847] py-2 text-[#006847]" onClick={() => signIn("facebook")}>
